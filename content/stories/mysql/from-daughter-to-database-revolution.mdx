...
# 从女儿的名字到数据库革命：MySQL的诞生

## 产品诞生

Monty Widenius 的，要从他的背景说起。

**自学成才的程序员**

Widenius 1962年出生于芬兰，在瑞典长大。他没有大学计算机学位，是自学成才的程序员。

从十几岁开始，他就沉迷于编程。他买了一台基于 CP/M 的电脑，开始学习汇编语言。1980年代，他开发了多个商业软件，包括一个用于库存管理的数据库系统 UNIREG。
...
1994年，一个客户需要一个 Web 前端来访问 UNIREG 数据。Widenius 尝试用 mSQL，但很快发现 mSQL 不支持 JOIN。
...
MySQL 采用 GPL（GNU General Public License）开源许可证发布，这意味着任何人都可以免费用、修改、分发 MySQL，但必须开源衍生作品。

但同时，MySQL AB（后来成立的公司）提供商业许可证。如果你的应用不想开源，可以买商业许可证。

这个模式的逻辑是：
- 个人开发者、开源项目、内部用：免费（GPL）
...
- **Yahoo!**：大量内部系统
- **Google**：AdWords 早期版本
- **Facebook**：2004年成立时选择 MySQL，至今仍大量用
- **YouTube**：视频元数据存储
- **Wikipedia**：MediaWiki 的底层数据库
- **Twitter**：早期版本
- **Alibaba**：淘宝、支付宝

这些公司说明 MySQL 可以处理大规模生产负载。

**LAMP 栈的统治**

到2005年，LAMP（Linux、Apache、MySQL、PHP/Python/Perl）成为 Web 开发的绝对主流。

根据各种估计，当时超过 70% 的网站用 MySQL 作为数据库。
